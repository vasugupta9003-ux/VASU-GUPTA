...
def customer_insights_tab(df: pd.DataFrame):
    ...
    with c1:
        edu = st.multiselect("Filter by Education",
                             options=sorted(df["Education"].unique()),
                             default=sorted(df["Education"].unique()))
    with c2:
        online = st.multiselect("Filter by Online (0=No,1=Yes)",
                                options=sorted(df["Online"].unique()),
                                default=sorted(df["Online"].unique()))
    with c3:
        cd = st.multiselect("Filter by CD Account (0=No,1=Yes)",
                            options=sorted(df["CD Account"].unique()),
                            default=sorted(df["CD Account"].unique()))
    fdf = df[(df["Education"].isin(edu)) &
             (df["Online"].isin(online)) &
             (df["CD Account"].isin(cd))]
    st.markdown("### 1️⃣ Income vs Education vs Loan")
    fig1 = px.box(fdf, x="Education", y="Income", color="Personal Loan",
                  points="outliers",
                  labels={"Income": "Annual Income ($000)"})
    st.plotly_chart(fig1, use_container_width=True)
    st.caption("- High-income Graduate & Advanced profiles are key prospects.")
    st.markdown("### 2️⃣ Credit Card Spending vs Loan")
    fig2 = px.violin(fdf, x="Personal Loan", y="CCAvg",
                     box=True, points="all",
                     labels={"CCAvg": "Avg Monthly Card Spend ($000)"})
    st.plotly_chart(fig2, use_container_width=True)
    st.caption("- Heavy spenders are strong candidates for personal loans.")
    st.markdown("### 3️⃣ Family Size & Online Usage vs Loan")
    fdf = fdf.copy()
    fdf["Family_Str"] = fdf["Family"].astype(str)
    fig3 = px.histogram(fdf, x="Family_Str", color="Personal Loan",
                        barmode="group", facet_col="Online",
                        labels={"Family_Str": "Family Size"})
    st.plotly_chart(fig3, use_container_width=True)
    st.caption("- Families of 3–4 using online banking convert better.")
    st.markdown("### 4️⃣ Product Holding Heatmap")
    pivot = (fdf.groupby(["CD Account", "Securities Account", "Online"])
               ["Personal Loan"].mean().reset_index())
    pivot["Conversion_Rate"] = (pivot["Personal Loan"] * 100).round(1)
    fig4 = px.density_heatmap(pivot,
                              x="CD Account", y="Online",
                              z="Conversion_Rate",
                              facet_col="Securities Account",
                              text_auto=True,
                              labels={"Conversion_Rate": "Loan Acceptance Rate (%)"})
    st.plotly_chart(fig4, use_container_width=True)
    st.caption("- CD / Securities + Online = highest cross-sell potential.")
    ...
    st.pyplot(fig5)
    st.caption("- Income, CCAvg & Education are strong positive drivers.")
...
def upload_and_predict_tab(base_df, stored_models):
    ...
    file = st.file_uploader("Upload CSV (similar structure to UniversalBank.csv)",
                            type=["csv"])
    if file is not None:
...
def main():
    st.set_page_config(page_title="Universal Bank - Personal Loan Dashboard",
                       layout="wide")
    st.title("Universal Bank | Personal Loan Conversion Analytics")
    st.write(
        "Use this dashboard as **Marketing Head** to discover high-probability segments, "
        "compare ML models, and score new leads."
    )
    try:
        df = load_data()
    except Exception:
    ...
    with st.expander("View Dataset Snapshot & Description"):
        ...
        st.markdown(
            "**Key Fields**: Personal Loan (target), Age, Experience, Income, "
    ...
    with tab2:
        ...
        if "metrics" in st.session_state:
            ...
            plot_roc_curves(st.session_state["models"],
                            st.session_state["X_test"],
                            st.session_state["y_test"])
            st.markdown("### Confusion Matrices")
            plot_confusion_matrices(st.session_state["models"],
